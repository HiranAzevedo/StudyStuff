...
sessionsRouter.post('/', function (request, response) { return __awaiter(void 0, void 0, void 0, function () {
    var _a, email, password, authService, _b, user, token;
    return __generator(this, function (_c) {
        switch (_c.label) {
            case 0:
                _a = request.body, email = _a.email, password = _a.password;
                authService = new AuthenticateUserService_1.default();
                return [4 /*yield*/, authService.execute({ email: email, password: password })];
            case 1:
                _b = _c.sent(), user = _b.user, token = _b.token;
                delete user.password;
                return [2 /*return*/, response.status(201).json({ user: user, token: token })];
        }
    });
}); });
